...
param(
    [string]$TestClass = "TestRunner",
    [string]$BranchName = "3-2-solution"
)
...
function Invoke-JavaTests {
    ...
    try {
        ...
        $compileCommand = "javac -cp `"src/test/java`""
        if ($mainFiles.Count -gt 0) {
            $compileCommand += " -cp `"src/main/java:src/test/java`""
            $compileCommand += " src/main/java/*.java"
        }
        $compileCommand += " src/test/java/*.java"
        
        $compileResult = Invoke-Expression $compileCommand 2>&1
        
        if ($LASTEXITCODE -ne 0) {
            return New-TestResult -AllTestsPassed $false -TotalTests 0 -PassedTests 0 -FailedTests 0 -BranchName $BranchName -ErrorMessage "Compilation failed: $compileResult"
        }
